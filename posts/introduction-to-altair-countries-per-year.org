...
#+OPTIONS: ^:{}
#+TOC: headlines 3
#+PROPERTY: header-args :session ~/.local/share/jupyter/runtime/kernel-d7fbe181-97a3-4039-b0ce-0c851d24a9f5-ssh.json
...
value_counts_chart = altair.Chart(year_counts).mark_bar().encode(
    ...
    y=Chart.count).properties(height=Chart.height, width=Chart.width)
...
#+begin_src python :results output :exports both
print("Altair counts(): {:,} bytes".format(ALTAIR_COUNTS_JS.stat().st_size))
print("Pandas value_counts: {:,} bytes".format(VALUE_COUNTS_JS.stat().st_size))
#+end_src
...
#+begin_src python :results none
plot = year_counts.hvplot.bar(x=Chart.year, y=Chart.count, height=Chart.height, width=800)
hvplot_chart = Chart.embed_hvplot(plot=plot, file_name="hvplot-bar")
output = hvplot_chart()
#+end_src
...
#+RESULTS:
#+begin_export html
<object type="text/html" data="hvplot-bar.html" style="width:100%" height=800>
  <p>Figure Missing</p>
</object>
#+end_export

#+begin_src python :results output :exports both
hvplot_path = Chart.path/"hvplot-bar.html"

print("Altair counts(): {:,} bytes".format(ALTAIR_COUNTS_JS.stat().st_size))
print("Pandas value_counts: {:,} bytes".format(VALUE_COUNTS_JS.stat().st_size))
print("value_counts using hvplot: {:,} bytes".format(hvplot_path.stat().st_size))
#+end_src

#+RESULTS:
: Altair counts(): 684,757 bytes
: Pandas value_counts: 1,711 bytes
: value_counts using hvplot: 12,082 bytes

At first glance it's a little surprising that the ~hvplot~ is so much bigger than the ~altair~ plot. This might be because it's embedding the whole HTML file like I used to do with altair... That's part of it, the other is that the bokeh has more stuff. Maybe adding interactivity to the altair plot would even them up.
...
** hvPlot
- hvPlot — hvPlot 0.9.2 documentation [Internet]. [cited 2024 Mar 22]. Available from: https://hvplot.holoviz.org/
