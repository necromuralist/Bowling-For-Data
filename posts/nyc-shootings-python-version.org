...
#+OPTIONS: ^:{}
#+TOC: headlines 3
#+PROPERTY: header-args :session ~/.local/share/jupyter/runtime/kernel-495a1695-6a31-4996-83a7-bb0f50de0df4-ssh.json
...
Reading the [[file:///home/hades/Downloads/NYPD_Shootings_Incident_Level_Data_Footnotes.pdf][Footnotes]] (link is to a PDF) it says:
...
boroughs = data.BORO.value_counts().reset_index().rename(
    columns=dict(index="Borough", BORO="Shootings"))
...
borough_monthly = monthly.agg({"BORO": "first",
                               "INCIDENT_KEY": "count"}).reset_index().rename(
                                   columns=dict(BORO="Borough", INCIDENT_KEY="Shootings"))

borough_monthly["Month"] = borough_monthly["date_time"].apply(
    lambda date: month_map[date.month])

chart = altair.Chart(borough_monthly).mark_line(
    point={"filled": False,
           "fill": "white"}).encode(
               x=altair.X("date_time"),
               y=altair.Y("Shootings"),
               color="Borough",
               tooltip=[
                   altair.Tooltip("Borough"),
                   altair.Tooltip("date_time", type="temporal"),
                   altair.Tooltip("Shootings")
               ]
).properties(
    width=800,
...
#+begin_src python :results output :exports both
locations = data.LOCATION_DESC.value_counts().reset_index().rename(columns=dict(index="Location", LOCATION_DESC="Count"))
