...
#+OPTIONS: ^:{}
#+TOC: headlines 3
#+PROPERTY: header-args :session ~/.local/share/jupyter/runtime/kernel-4f550039-1d70-49d3-aa85-6d7afcd087cf-ssh.json
...
So, the first thing we can notice is that the ~UNKNOWN~ counts don't match so sometims some attributes were known but not others, so they don't just represent cases where the perpetrator wasn't caught like I thought it might be. The secindo thin is that for age and race the unknowns have the highest counts. For all three categories the numbers of unknows are so high that it's hard to say how they migh affect the cases where the values are known.
